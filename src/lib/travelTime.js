...
import { fetchDriving } from '../api/kakaoMobility.js'
...
// 실제로 있다) 그 구간만 travelMin 근사치 + 직선으로 메우고 나머지는 실API 값을 쓴다. 전 구간이
// 다 실패했을 때만(키 미설정·인증 실패 등) null을 돌려줘서 호출부가 근사치로 완전히 폴백하게 한다.
// → { totalMinutes, legPaths: [[{lat,lng}, ...], ...] } | null
export async function estimateActualRoute(origin, orderedStops, travelMode) {
  const fetchLeg = travelMode === 'car' ? fetchDriving : travelMode === 'transit' ? fetchTransit : null
