...
## PR #68 코드리뷰 후 ruling (2026-09-02)

아래 ruling은 그 이전 결정(2026-09-01 ruling 포함)과 충돌할 때 우선한다.

- **공개 코드 — `share_code`로 되돌린다.** 공개 링크(`/s/:code`)는 `profiles.share_code` 전용 컬럼을
  쓰고, 클라이언트(`stampShare.js`)는 `ensure_share_code()` RPC로 지연 생성/조회한다.
  `get_public_stamp`은 `friend_code`를 더 이상 받지 않는다. 이유: 스탬프 카드를 SNS/단톡방에 공유하면
  그 코드가 곧 "친구 추가" 토큰이라, 신뢰하는 사람에게만 건네던 코드가 공개로 뿌려지는 동의 범위
  확대가 된다(PR #68 리뷰 지적). 기능 미출시라 깨질 friend_code 링크는 없다.
- **방문 인증의 구조적 한계 (문서화, 이번엔 최소 방어만).** `create_diary_entry`의 거리 검증은
  GPS 좌표(`p_lat/p_lng`)와 빵집 좌표(`p_bakery.lat/lng`) **둘 다 클라이언트가 준 값**을 대조한다.
  이 앱엔 빵집을 소유한 서버 테이블이 없어(외부 API 기반) 신뢰 가능한 빵집 좌표 출처가 없다.
  → RPC를 UI 없이 직접 호출해 두 좌표를 같게 넣으면 어떤 빵집이든 "인증"으로 만들 수 있다.
  영향: 본인 스탬프만 조작 가능(타 유저 데이터·금전 피해 없음).
  이번 조치: `verified`는 두 좌표가 **모두 대전 광역 범위(대략 lat 36.0~36.7, lng 127.0~127.9)**
  안일 때만 계산한다. `(0,0)` 류 자명한 위조는 막지만, 실제 대전 좌표를 아는 호출자의 위조는 못 막는다.
  완전 방어(서버가 `bakery_id`로 신뢰 좌표 조회 — 서버측 Kakao 조회 + 캐시 테이블)는 **후속 이슈**로 분리.
  이 한계를 알고, 이 기능에 금전/보상을 묶기 전에 먼저 해결한다.

## 최종 리뷰 후 ruling (2026-09-01) — 공개 코드 항목은 위 2026-09-02 ruling으로 대체됨

아래 ruling은 이 문서의 초기 브레인스토밍 결정과 충돌할 때 우선한다.

- **공개 코드**: ~~이슈 #63의 계약대로 새 링크는 `profiles.friend_code`를 재사용한다.~~
  → **철회.** 위 2026-09-02 ruling 참고(전용 `share_code`로 복귀).
