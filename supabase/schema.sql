...
-- ⚠️ 이 정의는 파일 끝 "이슈 #69" 섹션에서 대체된다(서버 bakery_coords 기준 검증).
-- 이 블록만 단독 실행하지 말 것 — 클라이언트 좌표 기준 게이트가 조용히 되살아난다.
...
-- Supabase 부트스트랩의 `alter default privileges ... grant all ... to anon, authenticated`
-- 때문에 SQL Editor 에서 만든 테이블은 그 grant 를 물려받는다. RLS 무정책이 이미 모든 행을
-- 막지만, 이 저장소는 어디서나 RLS 와 명시적 grant 를 함께 둔다 — 여기도 명시적으로 회수한다.
revoke all on bakery_coords from anon, authenticated;
...
begin
  ...
  -- btrim: resolve-bakery-coords 는 '{source}:{nativeId}' 를 canonical 키로 저장한다.
  -- 여기서도 앞뒤 공백을 떼어 저장/조회 키가 그 canonical 값과 일치하게 한다 — 패딩된
  -- 변형(' kakao:1', 'kakao:1 ')이 각각 다른 bakery_id + 다른 verified 슬롯이 되는 것을 막는다.
  v_bakery_id := btrim(p_bakery ->> 'id');
  ...
  --
  -- 구조적 한계(남아있음): #69 는 "아무 좌표쌍이나" 위조는 없앴지만, p_lat/p_lng 는 여전히
  -- 클라이언트가 보낸 값이고 빵집의 진짜 좌표는 앱에 공개돼 있다. 마음먹은 호출자는 GPS =
  -- 빵집 실제 좌표로 보내 verified=true 를 얻을 수 있다 — verified 는 물리적 방문의 확증이
  -- 아니다. 구 배치(스탬프 지도)도 아직 클라이언트 p_bakery.lat/lng 를 쓴다(별도 이슈).
  select lat, lng into v_bakery_lat, v_bakery_lng
    from bakery_coords
    where bakery_id = v_bakery_id;

  if v_bakery_lat is not null and v_bakery_lng is not null
    and v_visit_lat is not null and v_visit_lng is not null then
    v_distance_m := 6371000 * 2 * asin(sqrt(least(1, greatest(0,
      power(sin(radians(v_visit_lat - v_bakery_lat) / 2), 2)
      + cos(radians(v_bakery_lat)) * cos(radians(v_visit_lat))
      * power(sin(radians(v_visit_lng - v_bakery_lng) / 2), 2)
